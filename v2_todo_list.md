...
- [ ] 🟢 **(보류) 프로젝트 저장/불러오기 기능 부재**
  ...
  → **보류**: JSON 라이브러리(Jackson/Gson 등) 추가는 이 개발 환경에서 Maven Central 접근이 SSL 이슈로 불안정해 신규 의존성 도입 자체가 위험했고(테스트 의존성 다운로드 때 실제로 겪음), 라이브러리 없이 `java.io.Serializable` 기반으로 하려면 `DataTypeNode`/`DataTypeEntity`/`DataTypeElement` 등 모델 전반에 손을 대야 하는 큰 작업임. 저장/불러오기 메뉴 UI도 새로 설계해야 해서, 별도 세션에서 요구사항(파일 포맷, 저장 위치 등)을 먼저 정하고 진행하는 것을 권장.
...
- [ ] 🟢 **(보류) `DataTypeNode`에 부모 참조가 없어 트리 조작마다 BFS 전체 탐색**
  ...
  → **보류**: 이 도구가 다루는 트리 크기(사용자가 직접 입력하는 필드 수십~백여 개)에서는 BFS도 사실상 즉시 끝나 체감 성능 이득이 거의 없음. 반면 부모 포인터를 도입하려면 `addChildTo`/`addSiblingTo`의 raw list 조작을 모두 부모 포인터 동기화 로직으로 바꿔야 하는데, 같은 부모 내에서 형제 순서를 바꾸는 엣지 케이스(add 후 remove 순서에 의존하는 현재 동작)에서 포인터 정합성을 깨뜨리기 쉬움. GUI 조작 경로라 회귀를 눈으로 검증하기도 어려워, 위험 대비 이득이 낮다고 판단해 이번 범위에서는 다루지 않음.
...
- [ ] 🟢 **(보류) Hierarchy 재배치 UX가 좌클릭(포커스)+우클릭(대상)+모달 선택창 조합으로 비직관적**
  ...
  → **보류**: `TransferHandler` 기반 드래그앤드롭으로 전면 교체하는 것은 이 화면의 핵심 인터랙션(포커스/자식·형제 추가) 자체를 재설계하는 작업이고, GUI를 직접 조작해 보며 검증할 수 없는 이번 세션에서는 회귀 위험이 큼. UX 설계(드롭 위치별 동작 정의 등)를 먼저 정하고 실제 화면에서 확인하며 진행하는 것을 권장.
...
| 우선순위 | 완료 | 비고 |
|---|---|---|
| 🔴 P0 | 4 / 4 | 데이터 손상/오작동 직결 (XML 이스케이프, 테스트 부재, Type.NUMBER 오류, Hierarchy 무경고 초기화) |
| 🟡 P1 | 12 / 12 | 기능 완성도/신뢰성 개선 |
| 🟢 P2 | 15 / 18 | 코드 품질/UX 다듬기. 미완료 3건은 GUI를 직접 조작해 검증할 수 없는 이번 세션에서 진행하기엔 회귀 위험이 크거나(부모 포인터 도입, 드래그앤드롭 전환) 별도 요구사항 정의가 선행되어야 하는(프로젝트 저장/불러오기) 항목이라 의도적으로 보류함 — 각 항목에 "(보류)" 표시와 이유를 남겨둠 |

### 커밋 전 검증 방법

핵심 로직(`XsdGenerator`, `DataTypePipelineService`, `DataTypeFieldParser`, `FileSaver`, `DataTypeMeta`, `Occurrence`)에 대해 JUnit 5 단위 테스트 25개를 추가했고, 매 커밋마다 컴파일 + 전체 테스트 통과를 확인함. 이 개발 환경(Windows, 경로에 한글 포함)에서는 `gradle test` CLI 실행이 Gradle 테스트 워커의 클래스패스 이슈로 막혀 있어, JUnit Platform Launcher를 수동 구성해 테스트를 실행/검증함. GUI 동작 자체(버튼 클릭, 다이얼로그 노출 등)는 실제 화면에서 사용자가 한 번 확인해보는 것을 권장.
